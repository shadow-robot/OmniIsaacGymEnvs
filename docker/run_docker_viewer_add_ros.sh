...
docker exec -it isaac-sim-oige sh -c "/isaac-sim/python.sh -m pip install onnx onnxruntime"
docker exec -it isaac-sim-oige sh -c "apt install -y nano curl wget git highlight gnome-terminal lsb-release"
